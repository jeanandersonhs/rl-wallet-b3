"""
AgentQLearning — Agente Q-Learning Tabular para Gestão de Portfólio (B3)

Algoritmo off-policy de Temporal Difference (TD) control.
...
Onde:
    α  = learning rate (quanto o novo dado corrige a estimativa)
    γ  = discount factor (importância de recompensas futuras)
    r  = recompensa recebida do ambiente
    max_a' Q(s', a') = melhor valor futuro estimado

O estado contínuo (18 dimensões) é discretizado em bins para indexar
a Q-table (dicionário esparso via defaultdict).

Referência: documentacao.txt, seções 3, 4.1 e 9.
"""
...
class AgentQLearning:
    """
    Agente Q-Learning tabular para gestão de portfólio.
    ...
    def __init__(
        ...
        print(f"[Q-Learning] Calculando bins de discretização "
        ...
        print(f"[Q-Learning] Bins calculados para {len(self.bins)} features.")
    ...
    # ─────────────────────────────────────────────────────────────────────
    # ATUALIZAÇÃO Q (EQUAÇÃO DE BELLMAN)
    # ─────────────────────────────────────────────────────────────────────
    ...
    def update(
        ...
        """
        Atualiza Q(s, a) usando a equação de Bellman (TD update).
        ...
        if done:
            ...
            # Melhor valor futuro estimado (off-policy: usa max, não a')
        ...
        # Atualização incremental (Bellman update)
    ...
    def train(
        ...
        print(f"\n{'='*60}")
        print(f"  Q-LEARNING TREINAMENTO — {n_episodes} episódios")
        ...
        for episode in range(1, n_episodes + 1):
            ...
            while not done:
                ...
                # 3. Discretiza próximo estado
                next_state = self.discretize(next_state_continuous)

                # 4. Atualiza Q-table (equação de Bellman)
    ...
    def save(self, filepath: str) -> None:
        ...
        ----------
        filepath : str
            Caminho do arquivo para salvar (ex: 'resultados/q_agent.pkl').
        """
        data = {
        ...
        with open(filepath, "wb") as f:
            pickle.dump(data, f)
        print(f"[Q-Learning] Agente salvo em: {filepath}")
    ...
    def load(self, filepath: str) -> None:
        ...
        print(f"[Q-Learning] Agente carregado de: {filepath}")
